...
public class SendChangePhoneCode extends TlObject {
    ...
    public static final int ID = -2108208411;

    public String phone_number;
	public TlObject settings;

    public SendChangePhoneCode(String phone_number, TlObject settings) {
        this.phone_number = phone_number;
		this.settings = settings;
    }

    public static SendChangePhoneCode read(ByteArrayInputStream bytes) throws IOException {
        String phone_number = StringPrimitive.read(bytes).getValue();
		TlObject settings = TlObject.read(bytes);
		
		return new SendChangePhoneCode(phone_number, settings);
    }
