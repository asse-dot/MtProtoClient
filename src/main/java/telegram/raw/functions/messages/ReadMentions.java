...
public class ReadMentions extends TlObject {

    public static final int ID = 921026381;

    public TlObject peer;
	public int top_msg_id;

    public ReadMentions(TlObject peer, int top_msg_id) {
        this.peer = peer;
		this.top_msg_id = top_msg_id;
    }

    public static ReadMentions read(ByteArrayInputStream bytes) throws IOException {
        int flags = IntPrimitive.read(bytes).getValue();
		TlObject peer = TlObject.read(bytes);
		int top_msg_id = (flags & (1 << 0)) != 0 ?  IntPrimitive.read(bytes).getValue() : 0;
		
		return new ReadMentions(peer, top_msg_id);
    }
