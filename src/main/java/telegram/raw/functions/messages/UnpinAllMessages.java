...
public class UnpinAllMessages extends TlObject {

    public static final int ID = -299714136;

    public TlObject peer;
	public int top_msg_id;

    public UnpinAllMessages(TlObject peer, int top_msg_id) {
        this.peer = peer;
		this.top_msg_id = top_msg_id;
    }

    public static UnpinAllMessages read(ByteArrayInputStream bytes) throws IOException {
        int flags = IntPrimitive.read(bytes).getValue();
		TlObject peer = TlObject.read(bytes);
		int top_msg_id = (flags & (1 << 0)) != 0 ?  IntPrimitive.read(bytes).getValue() : 0;
		
		return new UnpinAllMessages(peer, top_msg_id);
    }
